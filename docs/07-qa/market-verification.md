...
## iOS에서 print()는 기기 로그에 도달하지 않는다

"준비 중"으로 굳은 구매 버튼의 원인을 좁히려고 실패 사유를 남기는
진단 코드를 넣고 TestFlight 빌드를 올렸다. **그 로그가 오지 않았다.**

```gdscript
print("[iap] unavailable %s" % reason)     # stdout — iOS 기기 로그에 안 온다
printerr("[iap] unavailable %s" % reason)  # stderr — 온다
```

`devicectl device process launch --console`로 붙어 상점 진입·복원·앱 종료까지
수행했는데 `[iap]` 줄이 한 줄도 오지 않았다. 같은 실행에서 Godot 엔진의
출력은 왔다.
...
엔진의 ERROR/WARNING은 stderr, `print()`는 stdout이다. 앱 종료 시 버퍼가
flush되기를 기대했으나 그것도 오지 않았다.

**진단 코드는 관측 가능한 경로로 내보내야 한다.** Android는 logcat이
stdout을 잡아 문제가 드러나지 않는다. 그래서 Android에서 잘 되던 방식을
그대로 iOS에 쓰면 조용히 아무것도 남지 않는다.

### 기기 로그 경로 정리

| 방법 | 되나 | 비고 |
|---|---|---|
| `idevicesyslog` | USB 필요 | 네트워크 기기는 `idevice_id -n`에 안 잡힌다 |
| `devicectl --console` | stderr만 | CoreDevice 터널. stdout은 오지 않는다 |
| Xcode Console (GUI) | 된다 | 사람이 읽어야 한다 |

`system_profiler SPUSBDataType`은 환경에 따라 출력이 비는 경우가 있다.
USB 연결 판정에는 `ioreg -p IOUSB`나 `idevicepair validate`를 쓴다.

### 더 큰 교훈

진단을 넣기 전에 **그 진단이 도달하는지부터 확인해야 한다.** 이번에는
진단 빌드를 올리고 나서야 진단 자체가 관측되지 않는다는 것을 알았고,
빌드 사이클을 한 번 더 썼다.
