...
### 시계열로 볼 때는 `hourlyActiveUsers`를 쓴다

`dailyActiveUsers`는 **직전 24시간 롤링** 값이다. 이걸 1시간마다 찍으면 이웃한 두 점이 창을 23/24 공유해서 곡선이 뭉개지고 시간대별 변화가 보이지 않는다.

`hourlyActiveUsers`는 창이 겹치지 않아 시간대별 굴곡이 그대로 남는다. 백오피스가 매시 정각에 이 값을 저장해 시계열 그래프로 그린다.

세션 TTL이 1시간이라 이 값이 **동시 접속에 가장 가까운 근사**지만 동시 접속은 아니다. heartbeat가 없다.

**백필은 불가능하다.** `lastSeenAt`은 덮어쓰기라 과거 시점의 활성 사용자 수를 복원할 수 없다. 시계열은 수집을 시작한 시점부터만 존재한다.
