...
func TestCrosswordPuzzleRegistryAuthBridgeContract(t *testing.T) {
	...
	// SDK 3.x는 web/private-web을 쓰지만, 게시 뒤에도 2.x 번들을 받는 클라이언트가
	// 남는다. 2026-09-19 apps/private-apps를 먼저 걷어냈다가 실사용자 preflight가
	// 403으로 떨어져 되돌렸다. 옛 호스트 트래픽이 0으로 관측될 때까지 네 개를 모두 둔다.
